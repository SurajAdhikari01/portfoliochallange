...
type CatBehavior = 'idle' | 'walking' | 'playing' | 'sleeping' | 'curious'
type CatDirection = 'left' | 'right'
type CatMood = 'happy' | 'playful' | 'tired' | 'curious'
...
export default function LucyTheCat() {
  ...
  const makeDecision = () => {
    // Determine if cat should respond to user inactivity
    ...
    if (timeSinceMoodChange > 15000 && Math.random() > 0.6) {
      const moods: CatMood[] = ['happy', 'playful', 'tired', 'curious']
  ...
  const moveToRandomPosition = () => {
    ...
    targetX.current = Math.max(50, Math.min(viewportWidth - 50, Math.random() * viewportWidth))
    targetY.current = Math.max(
      scrollPosition + 50,
      Math.min(scrollPosition + viewportHeight - 50, scrollPosition + Math.random() * viewportHeight)
    )
  ...
  const lookAround = () => {
    ...
    sequence.reduce((p, [dir, duration]) => 
      p.then(() => new Promise(resolve => {
        setDirection(dir as CatDirection)
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (behavior !== 'sleeping' && Math.random() < 0.05) {
        const distanceToMouse = Math.hypot(e.clientX - x.get(), (e.clientY + window.scrollY) - y.get())
  ...
  useEffect(() => {
    const visibilityCheck = () => {
      ...
      if (catPosY < scrollPos - 300 || catPosY > scrollPos + windowHeight + 300) {
  ...
  const handleCatInteraction = () => {
    ...
    if (behavior === 'sleeping') {
      ...
      // Happy thought
      const happyThoughts = ["Pet me more!", "Purrrr!", "That's the spot!", "I like you!"]
      setThinking(happyThoughts[Math.floor(Math.random() * happyThoughts.length)])
        ...
        >
          ...
          >
            ...
            <motion.ellipse
              ...
              transition={{ 
                repeat: behavior === 'curious' ? Number.POSITIVE_INFINITY : 0, 
                duration: 1.5, 
                repeatDelay: 1 
              }}
            />
            ...
            {/* Cat detail - whiskers */}
            <line x1="55" y1="33" x2="45" y2="30" stroke="#FFF" strokeWidth="0.7" />
            <line x1="55" y1="34" x2="45" y2="34" stroke="#FFF" strokeWidth="0.7" />
            <line x1="55" y1="35" x2="45" y2="38" stroke="#FFF" strokeWidth="0.7" />
            
            <line x1="75" y1="33" x2="85" y2="30" stroke="#FFF" strokeWidth="0.7" />
            <line x1="75" y1="34" x2="85" y2="34" stroke="#FFF" strokeWidth="0.7" />
            <line x1="75" y1="35" x2="85" y2="38" stroke="#FFF" strokeWidth="0.7" />
            ...
            {/* Cat mouth */}
            <motion.path
              d={behavior === 'sleeping' ? "M62 35 Q65 34 68 35" : "M62 36 Q65 38 68 36"}
              stroke="#000"
              strokeWidth="1"
              fill="none"
              animate={{ 
              ...
              transition={{ repeat: behavior === 'playing' ? Number.POSITIVE_INFINITY : 0, duration: 0.5 }}
            />
            ...
            <motion.path
              ...
              animate={{
                d: behavior === 'idle'
                  ? ["M15 40 Q5 20 15 10", "M15 40 Q5 30 15 10", "M15 40 Q5 20 15 10"]
                  : behavior === 'walking'
                    ? ["M15 40 Q5 20 15 10", "M15 40 Q0 30 10 15", "M15 40 Q5 20 15 10"]
                    : behavior === 'playing'
                      ? ["M15 40 Q0 10 20 5", "M15 40 Q20 15 5 10", "M15 40 Q0 10 20 5"]
                      : behavior === 'sleeping'
                        ? "M15 40 Q20 35 25 40"
                        : "M15 40 Q10 25 25 25"
              }}
            ...
            {behavior !== 'sleeping' && (
              <>
                <motion.line
                  ...
                  strokeWidth="4"
                  strokeLinecap="round"
                  animate={behavior === 'walking' ? { x2: [30, 35, 30], y2: [65, 60, 65] } : {}}
                ...
                <motion.line
                  ...
                  strokeWidth="4"
                  strokeLinecap="round"
                  animate={behavior === 'walking' ? { x2: [50, 45, 50], y2: [65, 60, 65] } : {}}
